...
public:
	...
	values[0] = ext_voltage;
	if (fabs(x[0])<0.4) values[0] = ext_voltage*(x[0]-0.4)/(-0.8) + int_voltage*(x[0]+0.4)/0.8;
	else if (x[0]>0) values[0] = int_voltage;
    }
private:
...
int main()
{
    ...
    File meshfile("./output/diode/mesh.pvd");
    File  Velocityfile("./output/diode/VelFinal.pvd");
    File  Pressurefile("./output/diode/PressFinal.pvd");
    File  Catfile("./output/diode/CatFinal.pvd");
    File   Anfile("./output/diode/AnFinal.pvd");
    File  phifile("./output/diode/phiFinal.pvd");
    ...
    double L_ref  = 1.0e-07;            //  length scale (m)
    double p_ref  = 1.0e+00;            //  ionic reference density (mol / m^3)
    double v_ref  = 1.0e-02;            //  velocity reference (m / s)
    ...
    fix_p      = fix_p / (n_avo * p_ref);       // dim'less fixed charge in P-doped material
    fix_n      = fix_n / (n_avo * p_ref);       // dim'less fixed charge in N-doped material
    surface_p  = surface_p / (L_ref * e_chrg * n_avo * p_ref ); // dim'less surface charge
    surface_n  = surface_n / (L_ref * e_chrg * n_avo * p_ref ); // dim'less surface charge
    ...
    printf("Define subdomains \n"); fflush(stdout);
      ...
      surfaces_pnp.set_all(1);
      inletoutlet.mark(surfaces_pnp,2);
      channelgate.mark(surfaces_pnp,3);
      dchannel.mark(surfaces_pnp,4);
      // L.ds = surfaces_pnp;
      // a.dx = subdomains_pnp;
      // L.dx = subdomains_pnp; 
    ...
    printf(" Define Dirichlet boundary condition\n"); fflush(stdout);
      ...
      if ( strcmp(meshIn,"box")==0) {
        ...
        DirichletBC bc_stokes_3(V,noflux,adapted_surfaces,4);
        DirichletBC bc_stokes_4(V,noflux,adapted_surfaces,5);
      
        bc_stokes.push_back(&bc_stokes_0);
        bc_stokes.push_back(&bc_stokes_1);
        bc_stokes.push_back(&bc_stokes_2);
        bc_stokes.push_back(&bc_stokes_3);
        bc_stokes.push_back(&bc_stokes_4);
      }
    ...
    while (done==false) {
        ...
        if ( strcmp(meshIn,"box")==0) {
            ...
            DirichletBC bc_stokes_r3(V,noflux,adapted_surfaces,4);
            DirichletBC bc_stokes_r4(V,noflux,adapted_surfaces,5);
            bc_stokes_r0.apply(A_stokes);
            bc_stokes_r1.apply(A_stokes);
            bc_stokes_r2.apply(A_stokes);
            bc_stokes_r3.apply(A_stokes);
            bc_stokes_r4.apply(A_stokes);
        }
    ...
    assemble(A,a); 
      ...
      if ( strcmp(meshIn,"box")==0) {
        ...
        // Read in boundaries from file
        DirichletBC bc_0r(W,noslip,adapted_surfaces,1);
        DirichletBC bc_1r(W,noslip,adapted_surfaces,2);
